...
public class TariffOptimizerRevoke extends TariffOptimizerBase {
  ...
  public TreeMap<Double, TariffSpecification> optimizeTariffs(
    ...
    List<TariffSpecification> suggestedSpecsToRevoke = 
        tariffSuggestionMaker.suggestTariffs(tariffSubscriptions,
            competingTariffs, marketManager, contextManager, me);
    
    log.info("revoke candidates:");
    for (TariffSpecification spec : suggestedSpecsToRevoke) {
      log.info(spec);
    }


    // utility computations - check option to merge them.
    ...
    List<TariffSpecification> dummy = new ArrayList<TariffSpecification>();
    HashMap<CustomerInfo, HashMap<TariffSpecification, Double>> 
    customer2RelevantTariffCharges = 
        estimateRelevantTariffCharges(
            dummy, tariffSubscriptions, competingTariffs,
            customer2ShiftedEnergy);

    // all possible tariff actions: {suggestedSpeces} U {no-op} a value of null
    // means no-op
    List<TariffSpecification> consideredTariffActions = new ArrayList<TariffSpecification>();
    consideredTariffActions.addAll(suggestedSpecsToRevoke); 
    consideredTariffActions.add(null);
    
    TreeMap<Double, TariffSpecification> sortedTariffs = 
        utilityEstimator.estimateRevokeUtilities(consideredTariffActions,
                                           tariffSubscriptions,
                                           competingTariffs,
                                           customer2RelevantTariffCharges,
                                           customer2ShiftedEnergy,
