...
implements BrokerContext
{
                            ...
                            String queueName, String serverQueue, long end)
  {
    ...
    if (null != jmsUrl)
      propertiesService.setProperty("edu.utexas.cs.tactex.core.jmsManagementService.jmsBrokerUrl",
                                      jmsUrl);
  ...
  class AgentRunner extends Thread
  {
    ...
    private void movePortfolioMgrToEnd(List<Activatable> services) {
      ...
      Activatable portolioMgr =(Activatable) 
          CollectionUtils.find(services, new Predicate() {
            @Override
